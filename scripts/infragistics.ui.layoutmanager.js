            ...
            */
            ...
            _analyzeGlWidth: function () {
                ...
                if (columnWidth && typeof columnWidth === "string") {
                    analyzedColumnWidth = this._analyzeGlColumnWidth(columnWidth, elWidth, gl.marginLeft);
                    gl.columnWidth = analyzedColumnWidth.columnWidth;
                    gl.columnWidthRatio = analyzedColumnWidth.columnWidthRatio;
                } else if (columnsWidth && $.isArray(columnsWidth)) {
                    for (i = 0; i < columnsWidth.length; i++) {
                        analyzedColumnWidth = this._analyzeGlColumnWidth(columnsWidth[i], elWidth, gl.marginLeft);
                        gl.columnsWidth[i] = analyzedColumnWidth.columnWidth;
                        gl.columnsWidthRatio.push(analyzedColumnWidth.columnWidthRatio);                        
                    }
                }

                //console.log("columns width");
                //console.log(gl.columnsWidth);

                //console.log("columns width ratio");
                //console.log(gl.columnsWidthRatio);
            },
            ...
            _analyzeGlHeight: function () {
                var i,
                    ...
                    columnsHeight = gl.columnsHeight,
                    elHeight = this.element.height(),
                    units,
                    analyzedColumnHeight;

                // Convert the column height option to number value
                if (columnHeight && typeof columnHeight === "string") {
                    analyzedColumnHeight = this._analyzeGlColumnHeight(columnHeight, elHeight, gl.marginTop);
                    gl.columnHeight = analyzedColumnHeight.columnHeight;
                    gl.columnHeightRatio = analyzedColumnHeight.columnHeightRatio;
                } else if (columnsHeight && $.isArray(columnsHeight)) {
                    for (i = 0; i < columnsHeight.length; i++) {
                        analyzedColumnHeight = this._analyzeGlColumnHeight(columnsHeight[i], elHeight, gl.marginTop);
                        gl.columnsHeight[i] = analyzedColumnHeight.columnHeight;
                        gl.columnsHeightRatio.push(analyzedColumnHeight.columnHeightRatio);
                    }
                }

                //console.log("columns height");
                //console.log(gl.columnsHeight);

                //console.log("columns height ratio");
                //console.log(gl.columnsHeightRatio);
            },
            ...
            _analyzeGlNotSetOptions: function () {
                ...
                if (!colsOption) {
                    if (rowsOption) {
                        ...
                        if (columnWidthOption) {
                            ...
                            // TODO: maybe take columnsWidth.length or columnsHeight.length
                ...
                // TODO: do this if columnsWidthOption is also not set
                if (!columnWidthOption) {
                ...
                // TODO: do this if columnsHeightOptoins is also not set
            ...
            _renderGlItemsFromItemsConfig: function (initialRendering) {
                ...
                for (i = 0; i < items.length; i++) {
                    ...
                    /* Cols width + height implementation */

                    if (gl.columnsWidth && gl.columnsWidth.length > 0 &&
                        gl.columnsHeight && gl.columnsHeight.length > 0) {

                        var itemWidth = this._calcGlItemSize(gl.columnsWidth, col, col + colSpan, this.element.width());
                        width = itemWidth + (colSpan - 1) * ml;
                        
                        var itemHeight = this._calcGlItemSize(gl.columnsHeight, row, row + rowSpan, this.element.height());
                        height = itemHeight + (rowSpan - 1) * ml;
                        
                        var itemLeft = this._calcGlItemSize(gl.columnsWidth, 0, col, this.element.width());
                        left = itemLeft + (col + 1) * ml;
                        
                        var itemTop = this._calcGlItemSize(gl.columnsHeight, 0, row, this.element.height());
                        top = itemTop + (row + 1) * mt;
                    } else {
                        /* Cols width + height implementation */
            ...
            // calculates width, height, top or left of an item,
            // handles '*'
            _calcGlItemSize: function (itemsSizes, from, to, containerSize) {
                var i,
                    j,
                    size = 0,
                    otherItemsSize = 0;
                ...
                for (i = from; i < to; i++) {
                    if (itemsSizes[i] === "*") {
                        otherItemsSize = 0;
                        for (j = 0; j < itemsSizes.length; j++) {
                            if (itemsSizes[j] !== "*") {
                                otherItemsSize += itemsSizes[j];
                            }
                        }

                        size += (containerSize - otherItemsSize);
                    } else {
                        size += itemsSizes[i];
                    }
                }
